...
Page({
  ...
  //获取借款时间
  bindPickerChangeGJJ: function(e){
    console.log('picker发送选择改变，携带值为', e.detail.value)
    this.data.lendTimeGJJ = Number(e.detail.value)
    if (1 <= Number(e.detail.value) && Number(e.detail.value)<= 5){
      this.setData({
        lendTnterest: 2.75
      })
    }
  ...
  bindPickerChange: function (e) {
    ...
    else if (2 <= Number(e.detail.value) && Number(e.detail.value) <= 5) {
      this.setData({
        lendTnterest: 4.75
      })
    }
  ...
  lendResultGJJ: function(){
    ...
    if (that.data.lendMoney == 0) {
      ...
      that.debxi(that.data.lendMoney, that.data.lendTimeGJJ, that.data.lendTnterest)
      that.debjin(that.data.lendMoney, that.data.lendTimeGJJ, that.data.lendTnterest)
      wx.navigateTo({
        url: '../result/result?lendTime=' + that.data.lendTimeGJJ + '&lendMoney=' + that.data.lendMoney + '&lixi_debx=' + that.data.lixi_debx + '&all_debx=' + that.data.all_debx + '&count_debx=' + that.data.count_debx + '&count_debjin=' + that.data.count_debjin + '&all_debjin=' + that.data.all_debjin + '&lixi_debjin=' + that.data.lixi_debjin + '&jian_debjin=' + that.data.jian_debjin + ''
      })
    }
